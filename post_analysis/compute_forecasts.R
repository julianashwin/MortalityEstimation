...
write.csv(forecasts_df, "data/results/bp_forecasts.csv", row.names = F)
...
fe_rolling_plt <-
  ...
  filter(age == 0 & n_ahead > 0) %>%
  ggplot() + theme_bw() + 
  scale_fill_manual("Models (rolling)", values = model_cols) +
  geom_bar(aes(x = n_ahead-1.5, y = sqrt(siler_fe2), fill = "Siler"), 
           stat = "summary", fun = mean, width = 0.5) +
...
err_full_plt <- 
  ...
  filter(age == 0) %>%
  ggplot() + theme_bw() + 
  scale_fill_manual("Model (full)", values = model_cols) +
  geom_bar(aes(x = est_year-1.5, y = sqrt(siler_err2), fill = "Siler"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year-1.0, y = sqrt(LC_err2), fill = "Lee-Carter"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year-0.5, y = sqrt(LC_SMM_err2), fill = "Lee-Carter (StMoMo)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+0, y = sqrt(LC_dt_err2), fill = "Lee-Carter (dt)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+0.5, y = sqrt(LC_dxt_err2), fill = "Lee-Carter (dxt)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+1.0, y = sqrt(LC_e0_err2), fill = "Lee-Carter (e0)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+1.5, y = sqrt(FDM_err2), fill = "FDM"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+2.0, y = sqrt(FDM_err2), fill = "CBD"), 
           stat = "summary", fun = mean, width = 0.5) +
  xlab("Est. year") + ylab("RMSE")+ ggtitle("In-sample") 

err_rolling_plt <- 
  tibble(forecasts_df) %>%
  filter(age == 0) %>%
  ggplot() + theme_bw() + 
  scale_fill_manual("Model (rolling)", values = model_cols) +
  geom_bar(aes(x = est_year-1.5, y = sqrt(siler_err2), fill = "Siler"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year-1.0, y = sqrt(LC_r_err2), fill = "Lee-Carter"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year-0.5, y = sqrt(LC_SMM_r_err2), fill = "Lee-Carter (StMoMo)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+0, y = sqrt(LC_dt_r_err2), fill = "Lee-Carter (dt)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+0.5, y = sqrt(LC_dxt_r_err2), fill = "Lee-Carter (dxt)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+1.0, y = sqrt(LC_e0_r_err2), fill = "Lee-Carter (e0)"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+1.5, y = sqrt(FDM_r_err2), fill = "FDM"), 
           stat = "summary", fun = mean, width = 0.5) +
  geom_bar(aes(x = est_year+2.0, y = sqrt(FDM_r_err2), fill = "CBD"), 
           stat = "summary", fun = mean, width = 0.5) +
  xlab("Est. year") + ylab("RMSE")+ ggtitle("In-sample") 

  
ggarrange(fe_full_plt,err_full_plt, nrow = 1, ncol = 2, common.legend = T, legend = "right")
ggarrange(fe_rolling_plt,err_rolling_plt, nrow = 1, ncol = 2, common.legend = T, legend = "right")

#ggsave("figures/forecasting/BP_forecast_errors.pdf", width = 8, height = 4)
rm(fe_full_plt,err_full_plt,fe_rolling_plt,err_rolling_plt)
...
ggplot(forecasts_df[which(forecasts_df$year %in% c(1903,1963,2018, 2048) &
  ...
  geom_line(aes(x = age, y = log(mx_LC), group = interaction(year, est_year), 
                color = "Lee-Carter"), alpha = 0.5) +
  geom_line(aes(x = age, y = log(mx_LC_SMM), group = interaction(year, est_year), 
                color= "Lee-Carter (StMoMo)"), alpha = 0.5) +
  geom_line(aes(x = age, y = log(mx_LC_dxt), group = interaction(year, est_year), 
                color = "Lee-Carter (dxt)"), alpha = 0.5) +
  geom_line(aes(x = age, y = log(mx_LC_e0), group = interaction(year, est_year), 
                color = "Lee-Carter (e0)"), alpha = 0.5) +
  geom_line(aes(x = age, y = log(mx_FDM), group = interaction(year, est_year), 
                color = "FDM"), alpha = 0.5) +
  geom_line(aes(x = age, y = log(mx_CBD_SMM), group = interaction(year, est_year), 
                color = "CBD"), alpha = 0.5) +
...
fe_plt <- ggplot(int_forecasts_df[obs,]) + theme_bw() + 
  scale_fill_manual("Model", values = model_cols) +
  geom_bar(aes(x = n_ahead-1, y = sqrt(siler_fe2), fill = "Siler"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead-0.75, y = sqrt(LC_fe2), fill = "Lee-Carter", alpha = "Full Sample"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead-0.5, y = sqrt(LC_dt_fe2), fill = "Lee-Carter (dt)", alpha = "Full Sample"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead-0.25, y = sqrt(LC_dxt_fe2), fill = "Lee-Carter (dxt)", alpha = "Full Sample"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead, y = sqrt(LC_e0_fe2), fill = "Lee-Carter (e0)", alpha = "Full Sample"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead+0.25, y = sqrt(LC_SMM_r_fe2), fill = "Lee-Carter", alpha = "Rolling"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead+0.5, y = sqrt(CBD_SMM_r_fe2), fill = "CBD", alpha = "Rolling"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead+0.75, y = sqrt(LC_dxt_r_fe2), fill = "Lee-Carter (dxt)", alpha = "Rolling"), 
           stat = "summary", fun = mean, width = 0.25) +
  geom_bar(aes(x = n_ahead+1, y = sqrt(LC_e0_r_fe2), fill = "Lee-Carter (e0)", alpha = "Rolling"), 
           stat = "summary", fun = mean, width = 0.25) +
